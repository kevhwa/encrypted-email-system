...
echo -e "\n***1. Making a request to changepw without a certificate.. This should fail.***\n"

echo "newpass" | ./bin/changepw -u $USER -p testuser

echo -e "\n***2. Making a request for a certificate with a bad password... This should fail.***\n"
...
echo -e "\n***12. User has a message waiting for them on the server... Attempt to changepw should fail.***\n"

username=$USER
echo "This is a mock message file" | sudo tee ../server-dir/mailboxes/$username/1010101
echo "testuser" | ./bin/changepw -u $USER -p tester

echo -e "\nClean Up: Resetting user's credentials to their original content...\n"

sudo rm -f ../server-dir/mailboxes/$username/1010101
echo "testuser" | ./bin/changepw -u $USER -p tester
